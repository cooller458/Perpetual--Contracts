...
contract SimpleOrderBook {
    ...
    function calcAmountToSell(
        ...
        require(reserveB > buyAmount, "Insufficient reserve for buy amount");
        return (reserveA * buyAmount) / (reserveB - buyAmount);
    }

    function calcAmountToBuy(
        address sellToken,
        address buyToken,
        uint256 sellAmount
        ...
        require(reserveA + sellAmount > 0, "Invalid sell amount or reserve");
    ...
    function buy(
        ...
        address buyToken,
        uint buyAmount
    ) external returns (uint256 soldAmount) {
